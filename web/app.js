...
/* 서버가 깨는 동안 아무 말도 안 하면 고장으로 읽힌다.
   API는 무료 티어라 15분 놀면 자고 깨는 데 50초쯤 걸리는데, 화면은
   즉시 떠 있어서 그 사이가 통째로 침묵이 된다.

   빠를 때는 띄우지 않는다 — 평소 요청은 100ms 안쪽이라 띠가 깜빡이면
   그게 더 시끄럽다. 2.5초를 넘긴 요청이 하나라도 있을 때만 띄운다. */
const WAKE_AFTER = 2500;
let inflight = 0, wokeAt = 0, wakeTimer = null;
...
function wakeTick() {
  ...
  const sec = `${Math.round(waited / 1000)}s`;
  const slow = inflight && waited >= WAKE_AFTER;

  // 일정 패널이 이미 '무엇을 하는 중'이라고 말하고 있으면 이유는 그
  // 상자 안에 적는다. 둘을 따로 띄우면 화면에서 겹쳐 둘 다 못 읽는다.
  const inPane = busy && !busy.hidden;
  if (inPane) {
    bar.hidden = true;
    $('#plan-busy-sec').textContent = slow ? sec : '';
    const sub = $('#plan-busy-sub');
    sub.textContent = slow ? tx('쉬고 있던 서버라 처음 한 번은 1분쯤 걸립니다.') : '';
    sub.hidden = !slow;
    return;
  }
  bar.hidden = !slow;
  if (slow) $('#waking-sec').textContent = sec;
}

function wakeStart() {
  if (!inflight++) wokeAt = Date.now();
  if (!wakeTimer) wakeTimer = setInterval(wakeTick, 500);
}
...
/* 단계마다 무엇을 하는 중인지 적는다. 메시지를 주지 않으면 앞의 것을
   그대로 둔다 — 깜빡이며 바뀌는 것보다 낫다. */
/* 붙어 있는 두 끼니만 함께 고를 수 있다.

   세 끼는 아침 7시부터 저녁 21시까지 14시간이고, 아침+저녁도 10시간이
   넘는다. 반나절 일정이 그 길이가 되면 더는 반나절이 아니다. 실제로
   돌려 보면 저녁을 못 채운 채 '찾지 못했습니다'만 남는다.

   눌리는데 안 되는 것보다 안 눌리는 편이 낫다. 왜 못 고르는지도 적는다. */
const MEAL_ORDER = ['breakfast', 'lunch', 'dinner'];

function mealAllows(picked, m) {
  if (picked.includes(m)) return true;          // 이미 고른 것은 늘 끌 수 있다
  if (picked.length >= 2) return false;
  return picked.every(x =>
    Math.abs(MEAL_ORDER.indexOf(x) - MEAL_ORDER.indexOf(m)) === 1);
}

function syncMeals() {
  const picked = S.meals || [];
  $$('#meal-seg button').forEach(b => {
    const m = b.dataset.meal;
    const ok = mealAllows(picked, m);
    b.disabled = !ok;
    b.classList.toggle('on', picked.includes(m));
    b.title = ok ? '' : tx(picked.length >= 2
      ? '끼니는 두 개까지 고를 수 있습니다.'
      : '떨어져 있는 두 끼니는 반나절에 다 넣을 수 없습니다.');
  });
}
...
function setLoading(on, msg) {
  ...
  const box = $('#plan-busy');
  if (!box) return;
  if (on && msg) $('#plan-busy-msg').textContent = tx(msg);
  if (!on) {
    $('#plan-busy-sub').hidden = true;
    $('#plan-busy-sec').textContent = '';
  }
  box.hidden = !on;
}
...
const WHY_FOOT = {
  ko: `거리·정보 충실도·알려진 정도·요즘 뜨는 정도·덜 붐비는 정도·취향을
    섞어 고릅니다. 어느 하나가 전부를 결정하지 않습니다.<br>
    &lsquo;요즘 뜨는&rsquo;은 작년 같은 달과 견준 관심의 변화입니다 &mdash;
    방문객 수가 아니라 새로 찾아보는 사람의 수입니다.`,
  en: `Chosen by mixing distance, how well documented it is, how known it is,
    how fast it is rising, how uncrowded it is, and your taste. No single one
    decides.<br>&lsquo;Trending&rsquo; is the change in attention against the
    same month last year &mdash; people newly looking it up, not visitors.`,
};
...
function whyBlock(c) {
  ...
  return `<div class="d-sec"><h4>선정 근거</h4>${rows}
    <p class="why-foot">${WHY_FOOT[S.lang] || WHY_FOOT.ko}</p></div>`;
}
...
/* 입력창 안내는 속성이라 텍스트 훑기가 못 본다. */
const AI_PLACEHOLDER = {
  ko: '어디서 얼마나 시간이 있으신가요?',
  en: 'Where are you, and how long do you have?',
};

function aiOpen(on) {
  const inp = $('#ai-input');
  if (inp) inp.placeholder = AI_PLACEHOLDER[S.lang] || AI_PLACEHOLDER.ko;
...
function savePlan() {
  ...
  const item = {
    ...
    saved: toLocalInput(seoulNow()).replace('T', ' '),
  };
  list.unshift(item);
  vaultSave(list);
  renderVault();
...
async function share() {
  ...
  const text = c && c.steps.length
    ? `서울 크레센도 · ${c.start}–${c.end} · ` +
      c.steps.map(s => s.title).join(' → ')
    : '서울 크레센도';
  try {
    if (navigator.share) {
      await navigator.share({ title:'서울 크레센도', text, url });
      return;
    }
...
async function start(usePrecise) {
  ...
  setLoading(true, usePrecise ? '위치를 확인하는 중…' : '주변을 찾는 중…');
  if (usePrecise) await locate();
  setLoading(true, '주변을 찾는 중…');
  await refreshWhere();
  await refresh();
  if (S.lang === 'en') {
...
/* '지금'은 서울의 지금이다.
   ...
   `new Date()`는 브라우저가 놓인 시간대를 준다. 기기가 서울 시간이 아니면
   시작 시각이 엉뚱하게 잡히고 그 시각으로 운영시간·혼잡·일정이 전부
   계산된다. 외국인이 쓸 서비스라 노트북이 집 시간대 그대로인 경우가 흔하다.

   한국은 1988년 이후 서머타임이 없어 UTC+9가 언제나 정확하다. 돌려주는
   Date의 지역 필드(getHours 등)를 읽으면 서울 벽시계가 나온다. */
const KST_MIN = 9 * 60;
...
/** 기기 시간대가 서울과 다른가. 다르면 화면에 그 사실을 적는다. */
function offSeoul() {
  return -new Date().getTimezoneOffset() !== KST_MIN;
}
...
/* 벽시계 그대로 찍는다. toISOString은 UTC로 바꿔 버려서, 시간대를 손으로
   더하고 빼는 방식은 한 번 더 어긋날 자리를 만든다. */
...
function bindUI() {
  ...
  if (startAt.getDate() !== now.getDate()) {
    ...
    // 기기 시간대가 다르면 시작 시각이 낯설게 보인다. 왜 그런지 적어 준다.
    geoNote('기기 시간대가 서울과 달라 서울 시각으로 맞췄습니다.');
  }

  $('#geo-allow').onclick = () => start(true);
  $('#geo-skip').onclick = () => start(false);
  $('#where-chip').onclick = async () => {
    setLoading(true, '위치를 확인하는 중…');
    await locate();
    setLoading(true, '주변을 찾는 중…');
    await refreshWhere();
    await refresh();
...
function tx(s) {
  ...
  // 문장 전체가 표에 있으면 거기서 끝난다. 단위를 먼저 돌리면 '1분'이
  // '1 min'이 되어 '…1분쯤 걸립니다.' 같은 통문장 열쇠가 안 맞는다.
  const whole = KO2EN[s.trim()];
...
// 쓸기 전의 원문. 언어를 되돌릴 때 이게 없으면 영어가 눌러앉는다.
const KO_ORIG = new WeakMap();

function restoreKorean(root) {
  (root || document.body).querySelectorAll('[data-ko-attr]').forEach(el => {
    JSON.parse(el.dataset.koAttr).forEach(([a, v]) => el.setAttribute(a, v));
  });
  const w = document.createTreeWalker(root || document.body, NodeFilter.SHOW_TEXT);
  while (w.nextNode()) {
    const n = w.currentNode;
    if (KO_ORIG.has(n)) n.nodeValue = KO_ORIG.get(n);
  }
}
...
function sweepKorean(root) {
  ...
  const keep = (el, attr, v) => {
    const had = el.dataset.koAttr ? JSON.parse(el.dataset.koAttr) : [];
    if (!had.some(([a]) => a === attr)) {
      had.push([attr, v]);
      el.dataset.koAttr = JSON.stringify(had);
    }
  };
  (root || document.body).querySelectorAll('[title]').forEach(el => {
    if (/[가-힣]/.test(el.title)) { keep(el, 'title', el.title); el.title = tx(el.title); }
  });
  ...
  ['placeholder', 'aria-label'].forEach(attr => {
    (root || document.body).querySelectorAll('[' + attr + ']').forEach(el => {
      ...
      if (/[가-힣]/.test(v)) { keep(el, attr, v); el.setAttribute(attr, tx(v)); }
    });
  });
  const w = document.createTreeWalker(root || document.body, NodeFilter.SHOW_TEXT);
  const jobs = [];
  while (w.nextNode()) {
    ...
    // `.t`는 장소 이름이라 옮기지 않는다. 다만 안에 배지가 같이 들어
    // 있어서 closest로 막았더니 '뜨는 중'·'오늘 마지막'까지 한국어로
    // 남았다. 지킬 것은 이름, 곧 `.t`의 직계 텍스트뿐이다.
    if (!host || host.classList.contains('t')
        || TX_SKIP.has((host.closest('[id]') || {}).id))
      continue;
    jobs.push(n);
  }
  jobs.forEach(n => {
    if (!KO_ORIG.has(n)) KO_ORIG.set(n, n.nodeValue);
    n.nodeValue = tx(n.nodeValue);
  });
}
...
function applyChrome() {
  ...
  // 문단은 굵은 글씨를 품고 있어 textContent로 바꾸면 한국어로 되돌릴 때
  // 강조가 사라진다. 원문 HTML을 간직한 채 통째로 갈아 끼운다.
  document.querySelectorAll('[data-en-html]').forEach(el => {
    if (!el.dataset.koHtml) el.dataset.koHtml = el.innerHTML;
    el.innerHTML = S.lang === 'en' ? el.dataset.enHtml : el.dataset.koHtml;
  });
  document.documentElement.lang = S.lang;
  if (S.lang === 'en') sweepKorean(); else restoreKorean();
}
