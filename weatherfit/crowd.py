...
서울시 실시간 도시데이터가 그 자리를 메운다.
    ...
    우리 콘텐츠 3,711건 중 2,856건(77.0%)이 반경 800m 안에 든다
...
def relief(got: dict | None) -> dict | None:
    ...
    붐빈다고만 말하고 끝내면 갈 곳을 잃는다. 시간을 옮길 수 있으면
    옮겨 준다 — 그게 분산이고, 이 서비스가 하려는 일이다.

    가라 마라를 정해 주지 않는다. 지금 몇 명이 있고 언제 한산해지는지를
    말하면 판단은 사용자가 한다. 사람마다 붐비는 곳을 찾아가는 이유가
    다르고, 그걸 대신 정해 줄 근거가 우리에게 없다.
    """
